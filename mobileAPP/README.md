...
- для `Text` и `Image` с `collectionField` значение берется из `item.fields[]` по совпадению `field.id === collectionField` (SSR-совместимый резолв, без чтения `item[collectionField]`).

### SSR parity по резолюции полей

Резолюция `collectionField` в RN синхронизирована с SSR:

- поиск поля: `findContentItemField(item, fieldId)` по `item.fields[].id`;
- display-значение для `Text`: `value_text -> value_boolean -> value_float -> value`;
- для `Image`: сначала `value_text`, затем `value` (string/object с `url`, `urls.small.url`, `urls.original.url`), иначе fallback на `src`/placeholder.

Это нужно, чтобы один и тот же JSON из builder вел себя одинаково в `site-runtime-ssr` и `mobileAPP`.
