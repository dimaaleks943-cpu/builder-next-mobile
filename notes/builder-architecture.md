...
- **Код-стиль для `builder`**
  - Везде используем **двойные кавычки** в TS/TSX.
  - В `sx` и стилях MUI используем полные имена свойств (`backgroundColor`, `justifyContent` и т.д.), не сокращения вроде `bgcolor`.
